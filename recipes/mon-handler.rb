...
if (Gem::Version.new(Chef::VERSION) < Gem::Version.new('0.10.9'))
  ...
  r = gem_package "chef-handler-datadog" do
    action :nothing
    version node["mon-agent"]["chef_handler_version"]
  end
...
chef_handler "Chef::Handler::Datadog" do
  ...
  arguments [
    ...
    :application_key => node['mon-agent']['application_key'],
    :use_ec2_instance_id => node['mon-agent']['use_ec2_instance_id']
  ]
