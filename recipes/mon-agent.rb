...
if node['mon-agent']['installrepo']
  include_recipe "mon-agent::repository"
end
...
if node['mon-agent']['install_base']
  package "datadog-agent-base" do
    version node['mon-agent']['agent_version']
  end
else
...
#
raise "Add a ['mon-agent']['api_key'] attribute to configure this node's Monitoring Agent." if node['mon-agent'] && node['mon-agent']['api_key'].nil?

template "/etc/mon-agent/mon-agent.conf" do
  owner "root"
  group "root"
  mode 0644
  variables(
  ...
  notifies :restart, "service[mon-agent]", :delayed
end
